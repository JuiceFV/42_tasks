...
void	pf_putnb(t_format *p)
{
	...
	if (p->f & F_LONG || p->f & F_LONG_LONG)
		n = (p->f & F_LONG_LONG) ? ((intmax_t)va_arg(p->ap, long long)) :
			((intmax_t)va_arg(p->ap, long));
	else if (p->f & F_SHORT || p->f & F_CHAR)
		n = (p->f & F_CHAR) ? (intmax_t)((char)va_arg(p->ap, int)) :
...
void	pf_putnb_base(int base, t_format *p)
{
	...
	if (p->f & F_LONG || p->f & F_LONG_LONG)
		n = (p->f & F_LONG_LONG) ? ((intmax_t)va_arg(p->ap, unsigned long long)) :
			((intmax_t)va_arg(p->ap, unsigned long));
	else if (p->f & F_SHORT || p->f & F_CHAR)
		n = (p->f & F_CHAR) ? (uintmax_t)((unsigned char)va_arg(p->ap, int))
...
void	itoa_printf(intmax_t n, t_format *p, int len)
{
	char		s[21];
	uintmax_t	tmp;
...
void	itoa_base_printf(uintmax_t n, int b, t_format *p)
{
	uintmax_t	tmp;
	char		s[21];
	int			ext;
...
void	itoa_base_fill(uintmax_t tmp, int b, char s[PF_BUF_SIZE], t_format *p)
{
	int		len;

	if (tmp && !(p->f & F_POINTER) && (p->f & F_ZERO) && (p->f & F_SHARP) &&
	b == 16 && !(p->f & F_LONG_LONG) && p->printed > 3)
		p->printed -= 2;
	len = p->printed;
