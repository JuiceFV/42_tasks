...
static void	parse_flags(t_format *p)
{
	while ((p->n = get_char_index("# +-0*", *p->format, -1)) > -1 && ++p->format)
		p->f |= (1 << p->n);
...
static void	field_width_precision(t_format *p)
{
...
static void	conversion_specifier(t_format *p)
{
	p->c = *p->format;
	if (p->c == 's')
		(p->f & F_LONG || p->f & F_LONG_LONG) ? pf_putwstr(p) : pf_putstr(p);
		...
		pf_putnb_base(get_char_index_lu(".b..ou..x", p->c, -1) << 1, p);
	else if (p->c == 'S')
		pf_putwstr(p);
	else if (p->c == 'p')
...
void		evaluate_optionals(t_format *p)
{
	...
	{
		if (*p->format == 'l')
			p->f |= (p->format[1] == 'l' && ++p->format) ? F_LONG_LONG : F_LONG;
		else if (*p->format == 'h')
			p->f |= (p->format[1] == 'h' && ++p->format) ? F_CHAR : F_SHORT;
		else if (*p->format == 'j')
			p->f |= F_INTMAX;
		else if (*p->format == 'z')
			p->f |= F_SIZE_T;
