...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/09 18:37:37 by cspider           #+#    #+#             */
/*   Updated: 2019/10/09 18:41:15 by cspider          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int		rtoa(t_vector *frmt, long double x, int b, t_specifiers *sp)
{
	int		res;
	double	frac;
	int		i;
	...
	res = 1 + itoa_base(frmt, (intmax_t)x, ABS(b), (b < 0));
	if (sp->quote)
		res += quote_div(frmt, ',');
	if (sp->precision == 0)
		return (res);
	...
	if (frac != 0)
		itoa_base(frmt, (long long)frac, ABS(b), 2 | (b < 0));
	return (res);
}
...
int		ev_f(t_specifiers *sp, t_vector *frmt, va_list ap)
{
	...
	return (rtoa(frmt, ABS(arg), 10, sp));
}
